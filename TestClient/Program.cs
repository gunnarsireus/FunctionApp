...
namespace TestClient
{
    class Program
    {
        ...
        {
            string endpoint = ConfigurationManager.AppSettings["eventGridEndpoint"];
            var httpClient = new HttpClient();
            httpClient.DefaultRequestHeaders.Add("aeg-sas-key",
                ConfigurationManager.AppSettings["eventGridKey"]);

            List<CustomEvent<Account>> events = new List<CustomEvent<Account>>();

            var customEvent = new CustomEvent<Account>();
            customEvent.EventType = @"Microsoft.EventGrid.SubscriptionValidationEvent";
